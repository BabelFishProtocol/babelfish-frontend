...
export const CurrencyInput = ({onChange, value, ...props}: IInputProps) => {
  // ToDo: Use from wei?
  const [valueTmp, setTmp] = React.useState<string>(value ? Web3.utils.fromWei(value).toString() : '');
  React.useEffect(
    () => {
...
const AmountInputUncontrolled = ({title, currencyText, onChange, value, decimalLimit, ...props}: any) => {
