...
import {BRIDGED_ADDRESSES, FROM_RSK_BRIDGE_ADDRESSES, TO_RSK_BRIDGE_ADDRESSES} from "./Contracts";
...
export async function bridgeFees(
  web3: Web3, sourceNetwork: chainEnum, goingInto: boolean
): Promise<BN> {
...
function getRedeemTrx(
  web3: Web3, sourceNetwork: chainEnum, asset: tokenType, quantity: BN, receiverAddress?: string,
): any {
  ...
  if (sourceNetwork === chainEnum.RSK) {
    return mAssetContract.methods.redeemTo(bAssetAddress, quantity, receiverAddress);
  }
  return mAssetContract.methods.redeemToBridge(bAssetAddress, quantity, receiverAddress);
}

export async function redeemEstimateGas(
  web3: Web3, sourceNetwork: chainEnum, asset: tokenType, quantity: BN,
): Promise<BN> {
...
export async function redeem(
  ...
  return new EthLiveTransaction(
    () => getRedeemTrx(web3, sourceNetwork, asset, quantity, receiverAddress).send({from: account}),
    {
      currency: bAssetName,
      amount: quantity,
    },
